...
static void bulkEraseDevice(DEVICE_TEST_DATA_T *devData, bool useAddrMode, bool verify)
{
	SPIFI_ERR_T errCode;
...
static void rangeCheckEraseDevice(DEVICE_TEST_DATA_T *devData, bool useAddrMode)
{
	SPIFI_ERR_T errCode;
...
static void setMemModeVerify(SPIFI_HANDLE_T *pSpifi, bool newState)
{
...
void test_suiteLibHelperBattery(SPIFI_HANDLE_T *pSpifi)
{
	...
	if ((devData.numSubBlocks * devData.subBlockSize) != (devData.numBlocks * devData.blockSize)) {
...
void test_suiteLibEraseBattery(SPIFI_HANDLE_T *pSpifi, bool useAddrMode)
{
	DEVICE_TEST_DATA_T devData;
	uint32_t idx;
	uint32_t testBlock;
	...
	if (devData.caps & SPIFI_CAP_SUBBLKERASE ) {
		...
		uint32_t subBlock;
		uint32_t numSubBlocksInBlock;

		numSubBlocksInBlock = devData.blockSize / devData.subBlockSize;

		/* first start by writing a known pattern to a block. */
		DEBUGOUT("Verifing programmed block... ");
		test_suiteWriteBlock(pSpifi, testBlock, TEST_TX_BUFFER_ID, devData.pageSize);

		/* get the first sub-block in the block. */
		subBlock = spifiGetSubBlockFromBlock(pSpifi, testBlock);

		/* Read the sub blocks and verify they contain pattern */
		for (idx = 0; idx < numSubBlocksInBlock; ++idx) {
			errCode = spifiRead(pSpifi,
								spifiGetAddrFromSubBlock(pSpifi, (subBlock + idx)),
		...
		DEBUGOUT("Erasing subblock %d... ", (subBlock + 1));
		test_suiteEraseSubBlocks(pSpifi, (subBlock + 1), 1);
		test_suiteVerifySubBlockErased(pSpifi, (subBlock + 1));
		DEBUGOUT("Passed.\r\n");
...
void test_suiteDataBattery(SPIFI_HANDLE_T *pSpifi, bool enableQuadRead, bool enableQuadWrite)
{
	DEVICE_TEST_DATA_T devData;
	SPIFI_ERR_T errCode;
...
void test_suiteMemModeTestBattery(SPIFI_HANDLE_T *pSpifi, uint32_t baseAddr, bool enableQuadRead, bool enableQuadWrite)
{
...
void test_suiteFillDeviceWithAddressPattern(SPIFI_HANDLE_T *pSpifi, bool verbose)
{
	uint32_t idx;
	uint32_t maxWrite;
...
void test_suiteValidateDeviceWithAddressPattern(SPIFI_HANDLE_T *pSpifi, bool verbose)
{
	uint32_t idx;
	uint32_t maxRead;
	uint32_t address;
...
void test_suiteVerifySubBlockErased(SPIFI_HANDLE_T *pSpifi, uint32_t subBlockNumber)
{
	uint32_t idx;
	uint32_t *tempBuffer;
	uint32_t subBlockSize;
...
void test_suiteVerifyBlockErased(SPIFI_HANDLE_T *pSpifi, uint32_t blockNumber, bool verbose)
{
	uint32_t idx;
...
/* Verify entire device is erased */
void test_suiteVerifyDeviceErased(SPIFI_HANDLE_T *pSpifi, bool verbose)
{
	uint32_t idx;
	uint32_t numBlocks;
	uint32_t startTime = 0;
